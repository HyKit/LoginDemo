...
@interface ShowMovieViewController () <AVPlayerViewControllerDelegate> {
...
- (void)respondsToButton:(UIButton *)sender {
    // 1、获取媒体资源地址
    MovieModel *model = _modelArray[_index];
    NSString *urlString = model.playUrl;
    NSURL *sourceMovieURL = [NSURL URLWithString:urlString];

    NSLog(@"source url ####   %@", sourceMovieURL);
    
    
    [self downloadMp4WithUrl:sourceMovieURL];
...
//            NSURL *url = [NSURL fileURLWithPath:[[NSBundle mainBundle] pathForAuxiliaryExecutable:@"宣传资料.mp4"]];
            ...
            // 设置媒体源数据
            _playerViewController.player = [AVPlayer playerWithURL:sourceMovieURL];
            // 设置拉伸模式
...
//下载mp4
- (void)downloadMp4WithUrl:(NSURL *)url {
